...
export default function App() {
    ...
    if (recommendedQuery) {
      setMessage({
        ...
        onConfirm: () => {
          ...
          setRecommendedQuery("");
          setSelectedQueries([]);
          navigate("/search", { state: { query: recommendedQuery } });
        },
      });
    }
  };
  ...
  return (
    <div className="min-h-screen w-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 flex flex-col font-sans">
      ...
      >
        {loading && relatedQueries.length === 0 ? (
          ...
          <>
            ...
            <MindMapNode label={originalQuery} position={CENTER_POS} isCenter />
            {relatedQueries.map((item, idx) => (
              <MindMapNode
                key={item.text}
                label={item.text}
